namespace Mediator
{
	...
	public class TeamChatRoom : IChatRoom
	{
		...
		{
			foreach (TeamMember teamMember in _teamMembers.Values)
			{
		...
		{
			TeamMember teamMember = _teamMembers[to];
			teamMember?.Receive(from, message);
		}

		public void SendTo<T>(string from, string message) where T : TeamMember
		{
			foreach (TeamMember teamMember in _teamMembers.Values.OfType<T>())
			{
				teamMember.Receive(from, message);
			}
		}
